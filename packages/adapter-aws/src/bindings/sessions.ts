 ...
 * AWS Session Adapter
 *
 * Uses DynamoDB for session storage when configured, providing fast
 * key-value lookups for token validation. Falls back to in-memory
 * storage for local development.
 */

import type { SessionAdapter, SessionData } from '@cruzjs/core/sessions';

export class AWSSessionAdapter implements SessionAdapter {
  ...
  async store(session: SessionData): Promise<void> {
    if (this.dynamoTable) {
      // TODO: DynamoDB PutItem with pk=`session:${session.tokenHash}`, sk=session.id
  ...
  async retrieve(tokenHash: string): Promise<SessionData | null> {
    if (this.dynamoTable) {
      // TODO: DynamoDB GetItem pk=`session:${tokenHash}`
    }
  ...
  async retrieveById(id: string): Promise<SessionData | null> {
    if (this.dynamoTable) {
      // TODO: DynamoDB GetItem pk=`session-id:${id}`
    }
  ...
  async listByUser(userId: string): Promise<SessionData[]> {
    if (this.dynamoTable) {
      // TODO: DynamoDB Query GSI on userId
    }
