...
**Browser apps use HttpOnly cookies. Not Authorization headers.**

```
1. User logs in → server returns Set-Cookie: auth_token=...; HttpOnly; SameSite=Lax
2. Browser sends auth_token cookie automatically on every same-origin request
3. session.middleware.ts reads cookie (or Bearer for non-browser clients)
4. ctx.session populated from session lookup
5. org-context.middleware.ts reads X-Organization-ID header → attaches ctx.org
6. tRPC procedures check ctx.session and ctx.org
```

**Rules:**
- Do NOT call `storeSessionToken()` or write `document.cookie` from client code.
- Do NOT read tokens from `localStorage` — they are not there.
- Do NOT add `Authorization: Bearer` headers manually. Cookie travels automatically.
- Use `trpc.X.useQuery()` / `useMutation()` — `httpBatchLink` sends same-origin cookies.
- For mobile/CLI/API clients: use the JWT refresh-token flow + Bearer header.
- Cookie name: `auth_token`. Attributes: `HttpOnly; SameSite=Lax; Path=/; Max-Age=2592000` (+ `Secure` in prod).
