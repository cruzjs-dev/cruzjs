/**
 * GCP Multi-Database Setup
 *
 * Registers multiple database connections for Google Cloud deployments.
 * Supports Cloud SQL connection strings configured via environment variables.
 *
...
/**
 * Wire GCP database connections into the MultiDatabaseService.
 *
 ...
 *     { name: 'read-replica', role: 'replica', connectionString: process.env.DATABASE_URL_REPLICA },
 *   ],
 * });
 * ```
 */
export function setupMultiDatabase(
  ...
  for (const conn of config.connections) {
    ...
    // PostgreSQL driver integration is added (e.g., drizzle-orm/node-postgres).
    console.info(
      `[MultiDatabase] GCP connection "${conn.name}" (${conn.role}) configured. ` +
