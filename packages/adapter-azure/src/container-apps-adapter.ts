...
export interface AzureContainerAppsAdapterConfig {
  databaseUrl?: string;
  redisUrl?: string;
...
export class AzureContainerAppsAdapter implements RuntimeAdapter {
  ...
  private config: AzureContainerAppsAdapterConfig;
  private localQueues = new Map<string, MemoryQueueBinding>();

  constructor(config?: AzureContainerAppsAdapterConfig) {
    this.config = config || {};
  }
  async init(_context: unknown): Promise<void> {
    // Azure Container Apps deployments use PostgreSQL
  ...
  waitUntil(promise: Promise<unknown>): void {
    promise.catch((e) => console.error("[AzureCA] Background failed:", e));
  }
