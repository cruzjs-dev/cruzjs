/**
 * Azure Session Adapter
 *
 * Uses Azure Table Storage for session persistence when configured.
 * Table Storage provides fast key-based lookups suitable for session
 * validation. Falls back to in-memory storage for local development.
 */

import type { SessionAdapter, SessionData } from '@cruzjs/core/sessions';

export class AzureSessionAdapter implements SessionAdapter {
