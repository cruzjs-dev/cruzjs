/**
 * Azure Multi-Database Setup
 *
 * Registers multiple database connections for Azure deployments.
 * Supports Azure SQL / Cosmos DB connection strings via environment variables.
 *
...
/**
 * Wire Azure database connections into the MultiDatabaseService.
 *
...
export function setupMultiDatabase(
  ...
  for (const conn of config.connections) {
    ...
    // Azure SQL driver integration is added.
    console.info(
      `[MultiDatabase] Azure connection "${conn.name}" (${conn.role}) configured. ` +
