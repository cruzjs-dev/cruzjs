...
export class StorageService {
  ...
  constructor(
    @inject(R2StorageDriver) private r2Driver: R2StorageDriver,
    @inject(LocalStorageDriver) private localDriver: LocalStorageDriver,
    ...
    if (driverName === 'r2') {
      // Fall back to local storage when R2 is not available (e.g. local dev without wrangler)
