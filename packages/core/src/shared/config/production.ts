 ...
 * Optimized for performance and security on Cloudflare Workers
 */
const productionConfigRaw: Config = {
  app: {
    env: 'production',
    port: envVar.PORT,
    url: envVar.APP_URL,
 ...
 * Wrapped in try/catch for Cloudflare Workers compatibility where env vars
 * aren't available at module load time
 */
let _productionConfig: Config | null = null;

export const productionConfig: Config = new Proxy({} as Config, {
  get(_target, prop: string) {
    if (!_productionConfig) {
      try {
        _productionConfig = configSchema.parse(productionConfigRaw);
      } catch (e) {
        // During bundling, return raw config without validation
        _productionConfig = productionConfigRaw as Config;
      }
    }
    return _productionConfig[prop as keyof Config];
  },
});
