 ...
 * Similar to production but with more logging enabled
 * Deployed on Cloudflare Workers
 */
const stagingConfigRaw: Config = {
  app: {
    env: 'staging',
    port: envVar.PORT,
    url: envVar.APP_URL,
...
export const stagingConfig: Config = new Proxy({} as Config, {
  get(_target, prop: string) {
    if (!_stagingConfig) {
      try {
        _stagingConfig = configSchema.parse(stagingConfigRaw);
      } catch (e) {
        _stagingConfig = stagingConfigRaw as Config;
      }
    }
    return _stagingConfig[prop as keyof Config];
  },
});
